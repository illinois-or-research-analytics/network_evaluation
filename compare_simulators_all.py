...
prefix = args.prefix if args.prefix == '' else f'{args.prefix}_'
...
df_successes.to_csv(
    output_dir / f'{prefix}successes.csv',
    index=False,
    float_format='%.2f',
)
...
if len(selection) > 5:
    ...
    for i, (stat, stat_type, distance_type) in enumerate(selection):
        values = df[df['Stat'] == stat]
        ax = sns.boxplot(
            x='Stat',
            y='Distance',
            hue='Simulator',
        ...
        if stat in MINMAX_BOUNDED_SCALARS:
            lb, ub = MINMAX_BOUNDED_SCALARS[stat]
            ax.set_ylim(lb, ub)

        ax.set_xlabel('')
        ax.set_ylabel(MAP_DISTANCE.get(distance_type, distance_type))
...
fig.tight_layout()
fig.savefig(output_dir / f'{prefix}boxplot.pdf', bbox_inches='tight')
