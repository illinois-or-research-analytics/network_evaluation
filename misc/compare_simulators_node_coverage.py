...
node_coverage_results = dict()
comp_results = dict()
successes = dict()


for network_id in all_network_ids:
    node_coverage_results[network_id] = dict()
    ...
    for (name, root, resolution) in zip(names, roots, resolutions):
        ...
        for replicate_dir in (root / network_id / resolution).iterdir():
            ...
            ss_fp = replicate_dir / 'stats.json'
            if ss_fp.exists():
                with open(ss_fp, 'r') as f:
                    ss = json.load(f)
                    node_coverage = 1 - ss['n_onodes'] / ss['n_nodes']
                    node_coverage_results[network_id].setdefault(
                        name,
                        [],
                    ).append(node_coverage)

        node_coverage_results[network_id][name] = np.mean(
            node_coverage_results[network_id][name]
        ) if len(node_coverage_results[network_id][name]) > 0 else 0.0
    ...
    for (network_id, resolution), data in sim_dict.items():
        ...
        for sim_name, distance in data.items():
            df_list.append([
                ...
                network_resolution,
                distance,
                node_coverage_results[network_id][sim_name],
            ])
df = pd.DataFrame(
    df_list,
    columns=[
...
fig.tight_layout()
fig.savefig(output_dir / 'scatterplot_positive_scalar.pdf')
...
fig.legend(handles, labels, loc='upper center', ncol=len(names))
fig.tight_layout()
fig.savefig(output_dir / 'scatterplot_bounded_scalar.pdf')
