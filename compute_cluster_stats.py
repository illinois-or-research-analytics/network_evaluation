...
parser = argparse.ArgumentParser(description='Compute cluster stats from network and community files.')
parser.add_argument('--network', type=str, required=True, help='Path to the network file')
parser.add_argument('--community', type=str, required=True, help='Path to the community file')
...
for key in global_stats:
    pd.Series([global_stats[key]]).to_csv(output_dir / f"{key}.txt", index=False, header=False)
...
for key in node_stats:
    pd.Series(node_stats[key]).to_csv(output_dir / f"{key}.txt", index=False, header=False)
...
for key in cluster_stats:
    pd.Series(cluster_stats[key]).to_csv(output_dir / f"{key}.txt", index=False, header=False)
